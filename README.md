...
- `codex.reasoningEfforts`: stage-specific reasoning effort overrides for `plan`, `implement`, `reviewTest` (`plan: low` = fast planning mode)
...
- `CODEX_REASONING_EFFORT_PLAN` (optional; planning reasoning effort override; set `low` for fast planning)
