---
name: ponyrace
description: Use when the user invokes /ponyrace, asks to run a pony race, wants role ponies to discuss whether a requirement direction matches their request, or wants Ponytrail's CLI requirement discussion before implementation.
---
...
Run Ponytrail's CLI requirement discussion before implementation. The skill makes `/ponyrace ...` a chat trigger for the existing `ponytrail ponyrace` command; the CLI remains the source of truth for clarification, role-pony discussion, vote tallying, Judge summary, and human confirmation.
...
- If `ponytrail` is unavailable and this is not the local Ponytrail repo, say the CLI is unavailable and ask the user how they want to run it.
